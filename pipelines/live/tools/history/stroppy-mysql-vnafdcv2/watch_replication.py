...
p=Path(__file__).parent;d=p/'semisync';live=Path('/home/yaroher/devel/github/stroppy-io/stroppy-cloud/pipelines/live')
...
for name,c in state['cells'].items():
 ...
 while time.monotonic()<deadline:
  ...
  if all(ok):
   ...
   assert rs['Rpl_semi_sync_replica_status']=='ON';assert settings['rpl_semi_sync_source_wait_for_replica_count']=='1'
