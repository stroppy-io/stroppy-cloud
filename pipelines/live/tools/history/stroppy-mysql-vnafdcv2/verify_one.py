...
workload_health=mod('inspect_metrics').inspect(cell['run_id'],'t-stroppy-live',start=result['segments'][0]['started_at'],end=health_end,entities=['docker/'+cell['uuid']+'-'+c['name'] for c in inputcell['containers'] if c.get('scrape') and c['name'].endswith('-mysqld-exporter')])
