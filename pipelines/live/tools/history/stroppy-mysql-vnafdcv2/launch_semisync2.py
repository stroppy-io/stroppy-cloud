...
p=Path(__file__).parent;d=p/'semisync2'
for _ in range(60):
 if all(not json.loads((p/prior/(k+'-current.json')).read_text()) for prior in ['semisync','fixed-mariadb'] for k in ['vm','disk','network','subnet','sg']):break
 time.sleep(20)
