...
spec=json.load(open('/tmp/stroppy-patroni-inputs-v2/postgres-patroni-ha-17-simple.json'))
...
def render(s):
 ...
 return s
for c in cts:
 name='pat-local-'+c['name'];d=p/'local'/c['name'];d.mkdir(parents=True);d.chmod(0o755)
