import concurrent.futures, datetime, json, subprocess, time, os
...
def cell(item):
 ...
 nodes=[x['machine'] for x in spec['containers'] if x['name'].endswith(('-mysql','-mariadb'))]
 checked={};deadline=time.monotonic()+5400
 try:
  while time.monotonic()<deadline:
   r=run([CLI,'-n','t-stroppy-live','run','list','--jq','.'])
   if r.returncode:time.sleep(15);continue
   row=next((r for r in json.loads(r.stdout)['resources'] if r['ref']=='run/'+c['run_id']),None)
   if row and row['phase'] in ['Completed','Failed','Canceled']:break
   if row:
    for node in nodes:
     if node in checked:continue
     target=P/(name+'-'+node+'.sql.json')
     r=run(['python3',str(P/'probe_remote.py'),c['uuid'][:8]+'-'+node,str(target)])
     target.with_suffix('.attempt.log').write_text(r.stdout+r.stderr)
     if r.returncode==0:checked[node]=json.loads(target.read_text())
     else:break
    if len(checked)==len(nodes) and not (LIVE/(name+'.replication.json')).exists():
     dump(LIVE/(name+'.replication.json'),dict(status='passed',run_id=c['run_id'],checked_at=datetime.datetime.now(datetime.timezone.utc).isoformat(),source='read-only SQL through ordinary agent shell on all three YC members',checks=checked))
     print(name,'all three live replication SQL probes passed',flush=True)
   time.sleep(15)
  ...
  if len(checked)!=3:raise RuntimeError('live SQL replication checks incomplete')
...
try:
 for svc,ports in [('victoria-logs-victoria-logs-single-server','19428:9428'),('victoria-traces-vt-single-server','18043:10428')]:
  log=(P/(svc+'.pf.log')).open('a')
  pf.append(subprocess.Popen(['kubectl','--kubeconfig','/tmp/stroppy-mysql-vnafdcv2/kubeconfig','--context','stroppy-live-test','-n','observability','port-forward','svc/'+svc,ports],stdout=log,stderr=subprocess.STDOUT))
