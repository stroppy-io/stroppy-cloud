...
def cell(item):
 ...
 nodes=[x['machine'] for x in spec['containers'] if x['name'].endswith(('-mysql','-mariadb'))] if c['topology']=='galera' else []
  ...
  while time.monotonic()<deadline:
   ...
   if row:
    ...
    if nodes and len(checked)==len(nodes) and not (LIVE/(name+'.replication.json')).exists():
     ...
     print(name,'all cluster member SQL probes passed',flush=True)
   time.sleep(15)
