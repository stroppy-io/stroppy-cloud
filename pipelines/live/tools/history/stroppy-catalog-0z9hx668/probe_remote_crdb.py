...
main=(live/'cockroach_probe.py').read_text()
...
container=next(c for c in inputspec['containers'] if c['machine']==machine and c['name'].endswith('-cockroach'))
...
result={'agent':agent,'source':'Read-only pgwire SQL and local HTTP node status through ordinary agent; no Docker or privilege escalation','probe_sha256':hashlib.sha256(code).hexdigest(),'checks':rows[0]}
target.write_text(json.dumps(result,indent=2)+'\n');print(agent,'Cockroach membership passed')
