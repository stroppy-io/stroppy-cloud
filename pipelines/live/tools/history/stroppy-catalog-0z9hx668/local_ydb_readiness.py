...
try:
 ...
 for c in cts:
  ...
  else:args+=['--network',net,'--ip',ips[c['machine']],'--hostname',ips[c['machine']]]
 ...
 if '--readiness' in sys.argv:
  env=__import__('os').environ.copy();env['STROPPY_TEST_ENDPOINT']=render(spec['workload']['url']);env['STROPPY_TEST_IMAGE']='docker.stroppy.io/stroppy-io/stroppy@sha256:d1083cac1321793911e39b00043afd8167ed2aa69fec5c0429fa17c7ef2ce945'
  native='stroppy--1-managed-ydb-ready'
  assert docker('inspect',native,check=False).returncode!=0,'existing readiness container'
  containers.append(native)
  r=subprocess.run(['go','test','./internal/activities','-run','^TestLiveManagedReadiness$','-count=1','-v'],cwd='/home/yaroher/devel/github/stroppy-io/stroppy-cloud/pipelines',env=env,capture_output=True,text=True,timeout=180)
  (folder/'readiness-check.log').write_text(r.stdout+r.stderr);print('live readiness',r.returncode,(r.stdout+r.stderr)[-2500:],flush=True);r.check_returncode()
