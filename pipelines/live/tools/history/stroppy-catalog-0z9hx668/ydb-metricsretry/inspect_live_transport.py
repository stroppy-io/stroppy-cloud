...
result=json.loads((P/(name+'.segments-preview.json')).read_text())
...
print(json.dumps({'exporters':m['exporters'],'series_count':m['series_count'],'data_filesystems':m['data_filesystems'],'native_segments':len(n['segments']),'nonfinite_samples':m['nonfinite_raw_samples']},indent=2))
