...
for target in ['linter_fix','linter','tests']:
 with (p/('dedicatedauth-'+target+'.log')).open('w') as log:
...
try:
 ...
 with (p/'stroppy-native/dedicatedauth-integration.log').open('w') as log:rc=subprocess.run(['make','integration'],cwd=r,stdout=log,stderr=subprocess.STDOUT).returncode
...
with (p/'stroppy-native/dedicatedauth-image-build.log').open('w') as log:subprocess.run(['docker','build','-t',tag,str(p/'stroppy-native/image-build')],check=True,stdout=log,stderr=subprocess.STDOUT)
subprocess.run(['python3',str(p/'publish_native.py'),'dedicatedauth'],check=True)
