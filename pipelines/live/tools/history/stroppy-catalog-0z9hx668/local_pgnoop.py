...
def run(args,check=True):
 ...
 return r
try:
 run(['docker','run','-d','--name',name,'-e','PGNOOP_WORKERS=2',img]);created.append(name)
 for _ in range(30):
 ...
 proof={'status':'passed','scope':'local packaged pg-noop 0.1.2 with released binary, real pgwire and native Stroppy execute_sql; YC pending','measurements':metrics,'binary_sha256':want['binary_sha256'],'release_archive_sha256':want['archive_sha256'],'native_log_sha256':hashlib.sha256(log.encode()).hexdigest(),'tps':'not applicable: protocol-only blackhole'}
