...
def cell(item):
 ...
 nodes=[x['machine'] for x in spec['containers'] if x['name'].endswith('-cockroach')]
  ...
  while time.monotonic()<deadline:
   ...
   if row:
    for node in nodes:
     ...
     r=run(['python3',str(P.parent/'probe_remote_crdb.py'),c['uuid'][:8]+'-'+node,str(target),str(len(nodes)),c['version'],str(probe_input)])
    ...
    if nodes and len(checked)==len(nodes) and (not (LIVE/(name+'.replication.json')).exists() or json.loads((LIVE/(name+'.replication.json')).read_text()).get('run_id')!=c['run_id']):
     dump(LIVE/(name+'.replication.json'),dict(status='passed',run_id=c['run_id'],checked_at=datetime.datetime.now(datetime.timezone.utc).isoformat(),source='Read-only pgwire SQL and local HTTP through ordinary agents on every CockroachDB YC member; all catalog members Online at expected version',checks=checked))
     print(name,'all CockroachDB member SQL probes passed',flush=True)
   time.sleep(15)
