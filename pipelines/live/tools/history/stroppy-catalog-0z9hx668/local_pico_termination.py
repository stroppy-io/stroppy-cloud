...
P=Path(__file__).parent;source=Path(sys.argv[1]);spec=json.loads(source.read_text());name=source.stem;folder=P/('local-'+name+'-termination-'+str(time.time_ns()));folder.mkdir(mode=0o700,exist_ok=True)
...
try:
 ...
 for c in cts:
  ...
  if '--probe' in sys.argv:
   for label, probe in [('abrupt',str(P/'picodata_probe_before_terminate.py')),('terminate','/home/yaroher/devel/github/stroppy-io/stroppy-cloud/pipelines/live/picodata_probe.py')]:
    before=docker('logs',physical,check=False);count_before=(before.stdout+before.stderr).count('IO error: unexpected end of file')
    q=subprocess.run(['python3',probe,'--host',ips[c['machine']],'--members',str(len(cts)),'--version',name.split('-')[-2]],env=dict(os.environ,PGPASSWORD=c['env']['PICODATA_ADMIN_PASSWORD']),capture_output=True,text=True);q.check_returncode();time.sleep(.5)
    after=docker('logs',physical,check=False);delta=(after.stdout+after.stderr).count('IO error: unexpected end of file')-count_before
    print(label,'EOF delta',delta,flush=True)
    assert delta==(1 if label=='abrupt' else 0),(label,delta)
