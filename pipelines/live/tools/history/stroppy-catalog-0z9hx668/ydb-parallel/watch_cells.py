...
def cell(item):
 ...
 try:
  ...
  for attempt in range(3):
   ...
   # Only known, independently checked YDB diagnostics can be classified.
   # Unknown hashes, native errors, missing artifacts or unhealthy members stop here.
   diagnostic=P/(name+'.native-diagnostic.json')
   if diagnostic.exists() and json.loads(diagnostic.read_text()).get('status')=='passed':
    for command in [
     ['python3',str(P.parent/'download_cell_artifacts.py'),str(P),name],
     ['python3',str(P.parent/'inspect_ydb_candidates.py'),name,str(P)],
     ['python3',str(P.parent/'classify_ydb_logs.py'),str(P),name],
    ]:
     classified=run(command)
     with (P/(name+'.classification.log')).open('a') as log:log.write(classified.stdout+classified.stderr)
     classified.check_returncode()
   time.sleep(20)
  r.check_returncode()
...
with concurrent.futures.ThreadPoolExecutor(max_workers=3) as pool:list(pool.map(cell,S['cells'].items()))
