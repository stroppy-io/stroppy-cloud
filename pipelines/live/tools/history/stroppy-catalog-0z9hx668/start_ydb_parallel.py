...
p=Path('/tmp/stroppy-catalog-0z9hx668');d=p/'ydb-parallel';old=p/'ydb-channelretry';cli=str(p/'bin/graphenectl');s=json.loads((d/'state.json').read_text());v=json.loads((d/'suite.json').read_text());assert 'started_at' not in s;assert len(v['cells'])==v['concurrency']==3
while True:
 counts={k:len(json.loads((old/(k+'-current.json')).read_text())) for k in ['vm','disk','network','subnet','sg','ydb']}
 phase=subprocess.check_output([cli,'-n','t-stroppy-live','get','run/matrix-ydbchannels-09c363ed-ydb-mirror-3-dc-25.4','--jq','.status'],text=True,timeout=30).strip()
...
public=Path('/home/yaroher/devel/github/stroppy-io/stroppy-cloud/pipelines/live');(public/'ydb-parallel-quota-check.json').write_text(json.dumps({'status':'passed','checked_at':s['started_at'],'concurrent_stands':3,'reservations':proof},indent=2)+'\n')
