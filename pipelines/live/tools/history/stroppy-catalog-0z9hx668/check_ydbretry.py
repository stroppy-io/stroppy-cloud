...
for target in ['linter_fix','linter','tests']:
 with (p/('ydbretry-'+target+'.log')).open('w') as log:
...
try:
 ...
 with (p/'stroppy-native/ydbretry-integration.log').open('w') as log:rc=subprocess.run(['make','integration'],cwd=r,stdout=log,stderr=subprocess.STDOUT).returncode
...
with (p/'stroppy-native/ydbretry-image-build.log').open('w') as log:subprocess.run(['docker','build','-t',tag,str(p/'stroppy-native/image-build')],check=True,stdout=log,stderr=subprocess.STDOUT)
subprocess.run(['python3',str(p/'publish_native.py'),'ydbretry'],check=True)
