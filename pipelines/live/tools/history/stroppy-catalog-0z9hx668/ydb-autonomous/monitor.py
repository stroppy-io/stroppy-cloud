...
ledger=json.loads((P/"ledger.json").read_text()) if (P/"ledger.json").exists() else {}
while True:
 ...
 subprocess.run(['python3',str(P.parent/'refresh_progress.py')],capture_output=True,text=True,timeout=60)
 print(time.strftime('%H:%M:%S'),[(r['ref'],r['phase']) for r in runs], 'VMs',len(json.loads((P/'vm-current.json').read_text())),flush=True)
 if any(r['ref']=='run/'+state['run_id'] for r in runs) and all(r['phase'] in ['Completed','Failed','Canceled'] for r in runs):
  if all(not json.loads((P/(k+'-current.json')).read_text()) for k in ledger) and all((P/(name+'.verified.json')).exists() or (P/(name+'.check-error.json')).exists() for name in state['cells']):break
 time.sleep(20)
