...
code=Path('/home/yaroher/devel/github/stroppy-io/stroppy-cloud/pipelines/live/picodata_probe.py').read_bytes()
inputspec=json.loads(Path(sys.argv[5]).read_text())
machine=agent.split('-',1)[1]
container=next(c for c in inputspec['containers'] if c['machine']==machine and c['name'].endswith('-picodata'))
...
password=next(c['env']['PICODATA_ADMIN_PASSWORD'] for c in inputspec['containers'] if c['name'].endswith('-picodata'))
payload=base64.b64encode(json.dumps({'code':code.decode(),'password':password,'args':['probe','--members',sys.argv[3],'--version',sys.argv[4]]}).encode()).decode()
...
result={'agent':agent,'source':'Read-only pgwire SQL through ordinary agent, no Docker or privilege escalation','probe_sha256':hashlib.sha256(code).hexdigest(),'checks':rows[0]}
target.write_text(json.dumps(result,indent=2)+'\n');print(agent,'Picodata membership passed')
