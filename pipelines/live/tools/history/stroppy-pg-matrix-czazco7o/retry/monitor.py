...
out=Path('/tmp/stroppy-pg-matrix-czazco7o/retry')
suite=json.loads(Path('pipelines/live/suite-postgres-versions-retry.json').read_text())
rid=(out/'run-id').read_text()
 ...
 try:
  ...
  for cell in suite['cells']:
   ...
   if phase=='Running':
    ...
    if status!=last.get(cr):print(now,cell['id'],status.strip().splitlines()[1:4],flush=True);last[cr]=status
  ...
  if parent.get('phase') in ['Completed','Failed','Canceled']:
   ...
   if parent.get('phase')=='Completed':
    result=json.loads(cli('run','result',rid,'-o','json'));(out/'versions.result.json').write_text(json.dumps(result,indent=2));print('FINAL',result.get('total'),result.get('done'),result.get('failed'),flush=True)
   else:print('FINAL',parent.get('phase'),flush=True)
   break
