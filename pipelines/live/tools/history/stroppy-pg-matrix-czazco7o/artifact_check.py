...
root=pathlib.Path('pipelines/live');work=pathlib.Path('/tmp/stroppy-pg-matrix-czazco7o');rows=[];blobs=[]
...
checked=json.loads(subprocess.check_output([str(work/'check-blobs'),str(path)]))
(root/'postgres-versions.artifacts.json').write_text(json.dumps(dict(artifacts=rows,download_checks=checked),indent=2)+'\n')
