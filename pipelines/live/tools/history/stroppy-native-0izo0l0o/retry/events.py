...
for name,c in s["cells"].items():
 proc=subprocess.Popen([cli,"-n","t-stroppy-live","events","run/"+c["run_id"],"--jq","."],stdout=subprocess.PIPE,stderr=subprocess.DEVNULL);sel=selectors.DefaultSelector();sel.register(proc.stdout,selectors.EVENT_READ);buf=b"";deadline=time.monotonic()+5
