...
данных. Inline-файл может быть пустым; относительные подкаталоги сохраняются.
`files.ref` имеет вид `artifact/<name>` в namespace запуска и скачивается на
runner. Входной artifact остаётся внешним ресурсом и не удаляется cleanup прогона.
